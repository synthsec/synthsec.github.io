---
layout: post
title:  "Unitrends 9.x api_storage exploit"
...
Here it is: while true; do ps -eaF;  done | grep -i [A]AAA . 
...
![ueb1]({{ "/assets/ueb1.png" | relative_url }})
<<<<Insert ueb1.png>>>>
...
<<<<Codeview>>>>tcpdump icmp  <<<</Codeview>>>>
...
Knowing that the session uuid is stored in a postgres database the first thing is a SQL injection attack.  Again we will continue to leverage the SSH access for debugging purposes.  Some quick googling and find the stackexchange thread for enabling query logging for postgres. Link here:https://stackoverflow.com/questions/722221/how-to-log-postgresql-queries . We just need to locate postgresql.conf and set log_statement  to 'all' and then tail the appropriate file in pg_log.

<<<<Codeview>>>>
[root@VMware-UB ~]# find / -name  postgresql.conf
/usr/bp/schema_oldScripts/postgresql.conf
/usr/bp/schema/postgresql.con
[root@VMware-UB ~]# cp /usr/bp/schema/postgresql.conf /tmp/postgresql.conf.bak
[root@VMware-UB ~]# echo "log_statement = 'all'" >> /usr/bp/schema/postgresql.conf
[root@VMware-UB ~]# reboot

Broadcast message from root@VMware-UB
	(/dev/pts/3) at 10:03 ...

The system is going down for reboot NOW!

VULNHUB-S2-052:~ csmith$ ssh root@10.0.0.220
root@10.0.0.220's password: 
Last login: Thu Dec  7 08:58:08 2017 from 10.0.0.169

[root@VMware-UB ~]# find / -name pg_log
/backups/UnitrendsDataBase/pg_log
[root@VMware-UB ~]# cd /backups/UnitrendsDataBase/pg_log
[root@VMware-UB pg_log]# tail -f postgresql-
postgresql-Thu.log  postgresql-Tue.log  postgresql-Wed.log  
[root@VMware-UB pg_log]# tail -f postgresql-Thu.log 
NOTE FROM CALE - LOGGING NOT WORKING YET NEED TO FIX AND UPDATE
<<<</Codeview>>>>

This turns out to be quite noisy, though of course we can pipe to grep only the queries that touch the sessions tables.  It quickly is apparent that this work as expected, the applications is just grabbing the expiration column for the matching uuid from the user session.  From there it must be comparing the expiration against the current time.  We know this without reading any source code!

Let’s try some basic SQLi strings. We of course will have to Base64 encode the AuthToken. Sure enough the first and most textbook example works: ‘ OR 1=1 -- . This is great, we have an authorization bypass and a root level CMDi exploit.  Eventually we come to realize that this isn’t as reliable we hoped and the issue is the auth bypass.  Yes our basic string worked initially but we are dependant of two things: a valid session in the sessions table, the first session returned not being expired.  

Knowing how simple the query is and how it works we opted to just do a simple UNION query which should be 100% reliable.  The condition we really need to satisfy is the check the application does on the returned expiration timestamp. We came up with this string: ' UNION SELECT -1 -- .
